...
class CqCqspbGovCnSpider(scrapy.Spider):
    name = 'cq_cqspb_gov_cn'
    allowed_domains = ['cqspb.gov.cn']
    start_urls = ['http://www.cqspb.gov.cn/zwyw/zwxw/dtxw/yw/']
    city = '500000'
    area = '500106'
    suid = 67109
    spider_status = {}
    ...
    def parse(self, response):
        ...
        li_list = response.xpath('//ul[@class="pList01"]/li').getall()
        ...
        if len(cont_url_list) == 0:
            ...
            title = Selector(text=li).xpath('//a/text()').extract_first().strip()
    ...
    def parse_content(self, response):
        ...
        try:
            ...
            title = response.xpath('//meta[@name="ArticleTitle"]/@content').getall()
            ...
            item["con_time"] = time_stamp(time_text.strip().replace('[', '').replace(']', ''))
            item["link"] = url
            item["image"] = img_link
