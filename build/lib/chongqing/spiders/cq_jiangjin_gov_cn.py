...
class CqJiangjinGovCnSpider(scrapy.Spider):
    name = 'cq_jiangjin_gov_cn'
    allowed_domains = ['jiangjin.gov.cn']
    start_urls = ['http://www.jiangjin.gov.cn/zwxx_180/jjdt/']
    city = '500000'
    area = '500381'
    suid = 67121
    spider_status = {}
    ...
    def parse(self, response):
        ...
        li_list = response.xpath('//div[@class="list"]/a').getall()
        print("crawl li length:", len(li_list))
        cont_url_list = []
        for li in reversed(li_list):
            if not li:
                continue
            content_url = Selector(text=li).xpath('//./@href').extract_first()
            if content_url:
        ...
        if len(cont_url_list) == 0:
            ...
            title = Selector(text=li).xpath('//span[1]/text()').extract_first().strip()
    ...
    def parse_content(self, response):
        ...
        try:
            ...
            title = Selector(text=li_tag).xpath('//span[1]/text()').extract_first()
            time_text = Selector(text=li_tag).xpath('//span[2]/text()').extract_first()
