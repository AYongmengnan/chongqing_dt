...
class CqHcGovCnSpider(scrapy.Spider):
    name = 'cq_hc_gov_cn'
    allowed_domains = ['hc.gov.cn']
    start_urls = ['http://www.hc.gov.cn/zwyw/hctt/']
    city = '500000'
    area = '500382'
    suid = 67124
    spider_status = {}
    ...
    def parse(self, response):
        ...
        li_list = response.xpath('//div[@class="lbbox_n"]/ul/li').getall()
        ...
        if len(cont_url_list) == 0:
            ...
            title = Selector(text=li).xpath('//span[@class="nr"]/text()').extract_first().strip()
    ...
    def parse_content(self, response):
        ...
        try:
            ...
            title = Selector(text=li_tag).xpath('//span[@class="nr"]/text()').extract_first()
            time_text = Selector(text=li_tag).xpath('//span[@class="date"]/text()').extract_first()
